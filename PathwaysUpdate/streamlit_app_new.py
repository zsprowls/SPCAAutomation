...
@st.cache_data
def load_data():
    """Load and merge data from GitHub"""
    # Load Pathways data
    ...
    if inventory_df is None:
        return pathways_df  # Return just pathways if inventory fails
    
    # Clean up inventory data (skip first 2 rows if needed)
    if len(inventory_df) > 0 and 'AnimalNumber' not in inventory_df.columns:
        inventory_df = pd.read_csv(StringIO(requests.get(CSV_FILES['inventory']).text), skiprows=2)
    ...
    merged_df['Days in System'] = pd.to_numeric(merged_df['Days in System'], errors='coerce').fillna(0)
...
if st.session_state.df is None:
    st.error("Failed to load data. Please check the GitHub repository configuration.")
    st.stop()
...
# Footer
st.markdown("---")
st.markdown("*Data loaded from GitHub repository*") 